...
The **Auto Recovery Engine (ARE)** is a Java 21 / Spring Boot 3.5.13 multi-module Maven project that simulates a financial microservices ecosystem. It provides **fault injection endpoints** on every business service and includes a live `recovery-engine` module that receives Prometheus/Alertmanager signals, evaluates recovery rules, and executes remediation actions such as restart or circuit-breaker transitions. The system handles user onboarding, account management, payment processing, event-driven notifications, and closed-loop self-healing experiments.
...
└── services/
    ...
    │           └── logback-spring.xml
    │
    └── recovery-engine/             # Recovery orchestration service with alert intake and remediation executor
```
...
> Note: the current project documentation has also been aligned with the runtime experiment workflow. Existing benchmark scripts now live under `scripts/` as scenario-specific Bash drivers (`simulate_crash.sh`, `simulate_error_rate.sh`, `simulate_latency.sh`, and `simulate_memory_leak.sh`) instead of the older Python experiment harness.
